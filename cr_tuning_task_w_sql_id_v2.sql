...
BEGIN
  ...
  sts_task := DBMS_SQLTUNE.CREATE_TUNING_TASK( sql_id => 'ch94nguxznm7n' 
                                              ,time_limit => 7200
...
SET LINESIZE 100
SELECT DBMS_SQLTUNE.REPORT_TUNING_TASK(upper('TASK_7073')  , 'TEXT', 'ALL', 'ALL')
FROM DUAL;

rem Scripts pour implémenter les recommandations
SELECT dbms_sqltune.script_tuning_task(upper('TASK_7073'), 'ALL') FROM dual;
